...
describe('DraggableTaskList', () => {
  ...
  describe('Category Colors', () => {
    it('should pass category colors to tasks', () => {
      ...
      // The category color should be shown as priority label background
      const priorityLabel = screen.getByTestId('priority-label');
      expect(priorityLabel).toHaveStyle({ backgroundColor: '#FF0000' });
    });

    it('should show default gray when no category', () => {
      const tasks = [createMockTask({ id: 'task-1', categoryId: null })];
      ...
      // Default gray on priority label background
      const priorityLabel = screen.getByTestId('priority-label');
