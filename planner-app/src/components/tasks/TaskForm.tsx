...
export interface TaskFormProps {
  ...
  categories?: Category[];
  /** Default date for new tasks (defaults to today if not provided) */
 ...
 * Note: reminders are passed separately to the parent component to handle async creation
 */
function formDataToTaskInput(data: FormData, isEditMode: boolean): CreateTaskInput & { reminders?: CreateReminderInput[] } {
  const parsedPriority = parsePriority(data.priority);

  const input: CreateTaskInput & { reminders?: CreateReminderInput[] } = {
...
export function TaskForm({
  ...
  const createFormDataFromTask = (t: Task | null, defaultScheduledDate?: Date | null): FormData => ({
    title: t?.title || '',
    description: t?.description || '',
    priority: t ? `${t.priority.letter}${t.priority.number || ''}` : 'B',
    categoryId: t?.categoryId || '',
    scheduledDate: t?.scheduledDate || defaultScheduledDate || null,
  ...
  // Form state
  const [formData, setFormData] = useState<FormData>(() => createFormDataFromTask(task, defaultDate));
  ...
  useEffect(() => {
    if (task && task.id !== prevTaskIdRef.current) {
      ...
      setFormData(createFormDataFromTask(task, defaultDate));
      setErrors({});
      setTouched({});
    }
  }, [task, defaultDate]);
  ...
  // Handle reminders toggle - enables/disables reminders section
  const [showReminders, setShowReminders] = useState(false);
  const handleRemindersToggle = useCallback((checked: boolean) => {
