...
export function ColorPicker({
  ...
  // Get color name, handling legacy colors not in the palette
  const getColorName = (color: string): string => {
    if (color in CATEGORY_COLOR_NAMES) {
      return CATEGORY_COLOR_NAMES[color as CategoryColor];
    }
    return 'Custom';
  };

  const selectedColorName = getColorName(value);
  const isLegacyColor = !CATEGORY_COLORS.includes(value as CategoryColor);
  ...
  return (
    <div className="w-full" data-testid={testId}>
      ...
      <div className="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg">
        <div
          className="w-8 h-8 rounded-md ring-1 ring-gray-300"
          style={{ backgroundColor: value }}
          aria-hidden="true"
        />
        <div className="flex flex-col">
          <span className="text-sm font-medium text-gray-900">
            Current: {selectedColorName}
          </span>
          {isLegacyColor && (
            <span className="text-xs text-gray-500">
              Select a new color below
            </span>
          )}
        </div>
      </div>
