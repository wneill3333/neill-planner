 ...
 * @returns Date object set to local midnight, or Invalid Date if format is wrong
 *
 * @example
 * const date = parseISODateString('2024-01-15'); // Date at local midnight
 */
export function parseISODateString(dateStr: string): Date {
  // Validate format: YYYY-MM-DD
  const match = dateStr?.match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) {
    console.warn(`[parseISODateString] Invalid date format: ${dateStr}`);
