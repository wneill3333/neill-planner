...
describe('recurrenceUtils', () => {
  ...
  // =============================================================================
  // addExceptionWithDedup Tests
  // =============================================================================

  describe('addExceptionWithDedup', () => {
    it('should add a new exception to an empty array', () => {
      const result = addExceptionWithDedup([], new Date(2026, 1, 5));
      expect(result).toHaveLength(1);
      expect(result[0].toISOString().split('T')[0]).toBe('2026-02-05');
    });

    it('should add a new exception to an existing array', () => {
      const existing = [new Date(2026, 1, 5)];
      const result = addExceptionWithDedup(existing, new Date(2026, 1, 10));
      expect(result).toHaveLength(2);
    });

    it('should NOT duplicate an existing exception (same Date)', () => {
      const existing = [new Date(2026, 1, 5)];
      const result = addExceptionWithDedup(existing, new Date(2026, 1, 5));
      expect(result).toHaveLength(1);
    });

    it('should NOT duplicate when existing has different time', () => {
      const existing = [new Date(2026, 1, 5, 10, 30)];
      const result = addExceptionWithDedup(existing, new Date(2026, 1, 5, 14, 0));
      expect(result).toHaveLength(1);
    });

    it('should handle ISO string input', () => {
      const existing = [new Date(2026, 1, 5)];
      const result = addExceptionWithDedup(existing, '2026-02-10');
      expect(result).toHaveLength(2);
    });

    it('should NOT duplicate when adding ISO string that matches existing Date', () => {
      const existing = [new Date(2026, 1, 5)];
      const result = addExceptionWithDedup(existing, '2026-02-05');
      expect(result).toHaveLength(1);
    });

    it('should handle mixed Date and ISO string exceptions', () => {
      const existing = [new Date(2026, 1, 5), '2026-02-10T00:00:00.000Z'];
      const result = addExceptionWithDedup(existing, new Date(2026, 1, 15));
      expect(result).toHaveLength(3);
    });

    it('should deduplicate pre-existing duplicates in the array', () => {
      const existing = [
        new Date(2026, 1, 5),
        new Date(2026, 1, 5, 12, 0),
        '2026-02-05T00:00:00.000Z',
      ];
      const result = addExceptionWithDedup(existing, new Date(2026, 1, 10));
      expect(result).toHaveLength(2); // Only Feb 5 and Feb 10
    });

    it('should return Date objects suitable for Firebase', () => {
      const existing = ['2026-02-05T00:00:00.000Z'];
      const result = addExceptionWithDedup(existing, '2026-02-10');
  ...
  describe('generateRecurringInstances - Instance Properties', () => {
    ...
    it('should inherit parent properties', () => {
      // Instances inherit the priority LETTER but get number=0 for auto-numbering
      // The actual number is assigned by the selector based on existing tasks for that day
      const pattern: RecurrencePattern = {
        type: 'daily',
        interval: 1,
        daysOfWeek: [],
      ...
      instances.forEach((instance) => {
        ...
        // Instances inherit letter but get number=0 for auto-numbering by selector
        expect(instance.priority).toEqual({ letter: 'B', number: 0 });
