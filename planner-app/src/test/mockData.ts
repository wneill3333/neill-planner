...
export interface CreateMockNoteOptions {
  id?: string;
  userId?: string;
  title?: string;
  content?: string;
 ...
 * Matches the actual Note type from types/note.types.ts
 */
export function createMockNote(options: CreateMockNoteOptions = {}): Note {
  noteCounter++;
  const id = options.id ?? `note-${noteCounter}`;
  const now = new Date();
  const defaultDate = new Date('2024-01-15T00:00:00.000Z');
  ...
  return {
    id,
    userId: options.userId ?? 'user-1',
    title: options.title ?? `Test Note ${noteCounter}`,
