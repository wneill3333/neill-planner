...
import { syncFromGoogle } from '../googleCalendar/googleCalendarSlice';
...
export const eventSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    // ==========================================================================
    builder.addCase(syncFromGoogle.fulfilled, (state, action) => {
      const importedEvents = action.payload;
      for (const event of importedEvents) {
        // Add event to normalized storage
        state.events[event.id] = event;

        // Add to date index
        const dateString = event.startTime.toISOString().split('T')[0];
        if (!state.eventIdsByDate[dateString]) {
          state.eventIdsByDate[dateString] = [];
        }
        if (!state.eventIdsByDate[dateString].includes(event.id)) {
          state.eventIdsByDate[dateString].push(event.id);
        }
      }
    });
  },
