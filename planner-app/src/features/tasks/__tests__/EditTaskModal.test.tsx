 ...
 * - Rendering tests (open/closed states, title, AddTaskFormSimple rendering)
...
// Store the mock task data passed to AddTaskFormSimple
...
// Mock the AddTaskFormSimple component
vi.mock('../../../components/tasks/AddTaskFormSimple', () => ({
  AddTaskFormSimple: ({ onSubmit, onCancel, isSubmitting, categories, task }: any) => {
...
describe('EditTaskModal - Rendering', () => {
  ...
  it('should render AddTaskFormSimple with task data', () => {
    const task = createDefaultTestTask();
    ...
    // AddTaskFormSimple receives a partial task object with title, priority, categoryId, recurrence
    expect(capturedTaskFormProps?.task?.title).toBe(task.title);
    expect(capturedTaskFormProps?.task?.priority).toEqual(task.priority);
    expect(capturedTaskFormProps?.task?.categoryId).toBe(task.categoryId);
  });

  it('should pass task prop to AddTaskFormSimple for edit mode', () => {
    const task = createDefaultTestTask();
    ...
    // AddTaskFormSimple receives task prop which indicates edit mode
...
describe('EditTaskModal - Recurring Task Editing', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetMockCounters();
    capturedTaskFormProps = null;
  ...
  it('should show RecurringEditDialog when editing a recurring instance', async () => {
    const recurringParentTask = createMockTask({
      id: 'parent-1',
    ...
    const recurringInstance = createMockTask({
      id: 'parent-1#2026-02-03',
      title: 'Recurring Task',
    ...
    const preloadedState = {
      tasks: createMockTasksState({
        recurringParentTasks: { 'parent-1': recurringParentTask },
      }),
      categories: createMockCategoriesState(),
    };

    renderWithProviders(
      <EditTaskModal isOpen={true} onClose={() => {}} task={recurringInstance} />,
      { preloadedState }
    );

    // RecurringEditDialog should appear with its text
    expect(screen.getByText(/This is a recurring task/i)).toBeInTheDocument();
    expect(screen.getByText(/Edit this occurrence only/i)).toBeInTheDocument();
    expect(screen.getByText(/Edit all future occurrences/i)).toBeInTheDocument();

    // TaskForm should NOT be visible initially
    expect(screen.queryByTestId('edit-task-form')).not.toBeInTheDocument();
  });

  it('should show TaskForm after selecting "Edit this only"', async () => {
    const user = userEvent.setup();
    const recurringParentTask = createMockTask({
      id: 'parent-1',
      title: 'Recurring Task',
    ...
    // Click "Edit this occurrence only" button
    await user.click(screen.getByTestId('edit-this-only-button'));

    // RecurringEditDialog text should be gone
    expect(screen.queryByText(/This is a recurring task/i)).not.toBeInTheDocument();

    // TaskForm should now appear
    await waitFor(() => {
      expect(screen.getByTestId('edit-task-form')).toBeInTheDocument();
    });
  });

  it('should show TaskForm after selecting "Edit all future"', async () => {
    const user = userEvent.setup();
    const recurringParentTask = createMockTask({
      id: 'parent-1',
      title: 'Recurring Task',
    ...
    // Click "Edit all future occurrences" button
    await user.click(screen.getByTestId('edit-all-future-button'));
  ...
  it('should dispatch editRecurringInstanceOnly when submitting with thisOnly mode', async () => {
    const user = userEvent.setup();
    const onClose = vi.fn();
    const recurringParentTask = createMockTask({
      id: 'parent-1',
    ...
    // Mock the createTask service (called by editRecurringInstanceOnly)
    ...
    vi.mocked(tasksService.updateTask).mockResolvedValue({
      ...recurringParentTask,
      recurrence: {
        ...recurringParentTask.recurrence!,
        exceptions: [new Date('2026-02-03')],
      },
    });

    renderWithProviders(
      <EditTaskModal isOpen={true} onClose={onClose} task={recurringInstance} />,
      { preloadedState }
    );
    ...
    // Verify createTask was called (by editRecurringInstanceOnly thunk)
    await waitFor(() => {
  ...
  it('should dispatch editRecurringFuture when submitting with allFuture mode', async () => {
    const user = userEvent.setup();
    const recurringParentTask = createMockTask({
      id: 'parent-1',
      title: 'Recurring Task',
    ...
    // Mock the updateTask service (called by editRecurringFuture)
    ...
    // Select "Edit all future occurrences"
    await user.click(screen.getByTestId('edit-all-future-button'));
    ...
    // Verify updateTask was called (by editRecurringFuture thunk)
    await waitFor(() => {
      expect(tasksService.updateTask).toHaveBeenCalled();
    });
  });

  it('should NOT show RecurringEditDialog for non-recurring tasks', async () => {
    ...
    renderWithProviders(
      <EditTaskModal isOpen={true} onClose={() => {}} task={regularTask} />,
      { preloadedState: getDefaultPreloadedState() }
    );

    // TaskForm should appear immediately
    expect(screen.getByTestId('edit-task-form')).toBeInTheDocument();

    // RecurringEditDialog text should NOT be present
    expect(screen.queryByText(/This is a recurring task/i)).not.toBeInTheDocument();
    expect(screen.queryByText(/Edit this occurrence only/i)).not.toBeInTheDocument();
  });

  it('should close modal when canceling from RecurringEditDialog', async () => {
    const user = userEvent.setup();
    const handleClose = vi.fn();
    const recurringParentTask = createMockTask({
      id: 'parent-1',
    ...
    renderWithProviders(
      <EditTaskModal isOpen={true} onClose={handleClose} task={recurringInstance} />,
      { preloadedState }
    );

    // Click Cancel on RecurringEditDialog
    await user.click(screen.getByTestId('cancel-button'));
