...
const mockGetRecurringTasks = vi.mocked(tasksService.getRecurringTasks);
...
describe('TaskListContainer', () => {
  ...
  describe('Task Rendering', () => {
    ...
    it('should show category colors on priority label', () => {
      const user = createMockUser();
    ...
    it('should show default gray when no category', () => {
      const user = createMockUser();
      const task = createMockTask({ id: 'task-1', title: 'Task', categoryId: null });
      ...
      // Default gray on priority label when no category (uncategorized color)
      const priorityLabel = screen.getByTestId('priority-label');
    ...
    it('should cycle task status when status symbol is clicked', async () => {
      ...
      await waitFor(() => {
        expect(mockUpdateTask).toHaveBeenCalledWith(
          ...
          'user-1' // userId is now passed correctly
        );
      });
    });
