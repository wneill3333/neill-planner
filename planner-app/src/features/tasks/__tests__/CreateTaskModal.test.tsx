...
// Mock the AddTaskFormSimple component (simplified form used in redesign)
vi.mock('../../../components/tasks/AddTaskFormSimple', () => ({
  AddTaskFormSimple: ({ onSubmit, onCancel, isSubmitting, categories }: any) => {
    const handleSubmit = () => {
...
describe('CreateTaskModal - Rendering', () => {
  ...
  it('should render AddTaskFormSimple inside modal', () => {
    renderWithProviders(<CreateTaskModal isOpen={true} onClose={() => {}} />);

    expect(screen.getByTestId('add-task-form')).toBeInTheDocument();
  });

  it('should pass categories to AddTaskFormSimple', () => {
    const _categories = [
    ...
    // AddTaskFormSimple should receive categories from Redux store
    expect(screen.getByTestId('add-task-form')).toBeInTheDocument();
  });
