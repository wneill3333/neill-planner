...
import { NONE_CATEGORY, NONE_CATEGORY_ID } from '../../types';
...
/**
 * Stable reference for None category entity (module-level constant).
 * Created once to avoid re-creating the object on every selector call.
 */
const NONE_CATEGORY_ENTITY: Category = {
  id: NONE_CATEGORY_ID,
  userId: '', // Virtual category has no owner
  name: NONE_CATEGORY.name,
  color: NONE_CATEGORY.color,
  sortOrder: NONE_CATEGORY.sortOrder,
  createdAt: new Date(0), // Epoch time for virtual category
...
export const categorySlice = createSlice({
  ...
  reducers: {
    ...
    /**
     * Reorder categories locally (optimistic update)
     * Updates both categoryIds order and individual sortOrder values
     */
    reorderCategoriesLocal: (state, action: PayloadAction<string[]>) => {
      const newOrder = action.payload;

      // Update categoryIds to new order
      state.categoryIds = newOrder;

      // Update sortOrder for each category
      for (let i = 0; i < newOrder.length; i++) {
        const categoryId = newOrder[i];
        if (state.categories[categoryId]) {
          state.categories[categoryId].sortOrder = i;
        }
      }
    },
  },
  extraReducers: (builder) => {
    ...
    // ==========================================================================
    builder
      .addCase(reorderCategoriesAsync.pending, (state) => {
        state.syncStatus = 'syncing';
      })
      .addCase(reorderCategoriesAsync.fulfilled, (state) => {
        // State already updated via reorderCategoriesLocal (optimistic update)
        state.syncStatus = 'synced';
      })
      .addCase(reorderCategoriesAsync.rejected, (state, action) => {
 ...
 * Includes the virtual "None" category as the first item.
...
export const selectAllCategories = createSelector(
  ...
  (categoryIds, categories): Category[] => {
    ...
    // Return None category first, followed by user categories
