...
describe('Category Thunks', () => {
  ...
  describe('deleteCategory', () => {
    it('should handle pending state', () => {
      ...
      vi.mocked(tasksService.getTasksByCategory).mockResolvedValue([]);
    ...
    it('should delete category successfully', async () => {
      ...
      // Mock tasks service to return no tasks with this category
      vi.mocked(tasksService.getTasksByCategory).mockResolvedValue([]);
    ...
    it('should return affected task IDs when deleting category with tasks', async () => {
      const existingCategory = createMockCategory({ id: 'cat-1' });
      const store = createTestStore({
        categories: { 'cat-1': existingCategory },
        categoryIds: ['cat-1'],
      ...
      // Mock tasks service to return tasks with this category
      const mockTasks = [
        { id: 'task-1', categoryId: 'cat-1' },
        { id: 'task-2', categoryId: 'cat-1' },
      ];
      vi.mocked(tasksService.getTasksByCategory).mockResolvedValue(mockTasks as any);
      vi.mocked(tasksService.batchUpdateTasks).mockResolvedValue(undefined);
      vi.mocked(categoriesService.deleteCategory).mockResolvedValue();

      const result = await store.dispatch(
        deleteCategory({ categoryId: 'cat-1', userId: 'user-1' })
      );

      // Verify the return payload includes affected task IDs
      expect(result.payload).toEqual({
        categoryId: 'cat-1',
        affectedTaskIds: ['task-1', 'task-2'],
      });

      // Verify tasks were updated in Firestore
      expect(tasksService.batchUpdateTasks).toHaveBeenCalledWith(
        [
          { id: 'task-1', categoryId: null },
          { id: 'task-2', categoryId: null },
        ],
        'user-1'
      );
