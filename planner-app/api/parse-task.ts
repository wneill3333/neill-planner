...
async function parseWithClaude(
  ...
  // Validate and coerce the parsed response
...
function validateParsedResponse(parsed: unknown): ParsedTaskData {
  ...
  // Start time: string in HH:MM format or null
...
function setCorsHeaders(req: VercelRequest, res: VercelResponse): void {
  const allowedOrigins = [
    'https://neill-planner.vercel.app',
    'http://localhost:5173',
    'http://localhost:3000',
  ];
...
export default async function handler(
  ...
  try {
    const parsedTask = await parseWithClaude(query, categories, selectedDate);
    ...
    // Check for Anthropic API errors (rate limit, auth, etc.)
    if (
